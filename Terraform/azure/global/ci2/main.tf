...
data "azurerm_public_ip" "pip_ci2" {
  name                = "pip_ci2"
  resource_group_name = "${data.azurerm_resource_group.rg_terraform.name}"
}

resource "azurerm_network_interface" "vnic_ci2" {
  name                      = "vnic_ci2"
  ...
  ip_configuration {
    ...
    public_ip_address_id          = "${data.azurerm_public_ip.pip_ci2.id}"
  }
}

resource "azurerm_virtual_machine" "ci2" {
  name                  = "${var.ci_machine_name_suffix}"
  location              = "${data.azurerm_resource_group.rg_terraform.location}"
  resource_group_name   = "${data.azurerm_resource_group.rg_terraform.name}"
  network_interface_ids = ["${azurerm_network_interface.vnic_ci2.id}"]
  ...
  storage_os_disk {
    name              = "disk_os_${var.ci_machine_name_suffix}"
    caching           = "ReadWrite"
...
resource "null_resource" "bootstrap" {
  depends_on = ["azurerm_virtual_machine.ci2"]
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      while ($true) {
        try {
          $pssession = New-PSSession -ComputerName "${data.azurerm_public_ip.pip_ci2.ip_address}" -Credential $cred -ErrorAction Stop
        } catch {
...
output "terraform_public_fqdn" {
  value = "${data.azurerm_public_ip.pip_ci2.domain_name_label}.${data.azurerm_resource_group.rg_terraform.location}.cloudapp.azure.com"
}

output "terraform_public_ip_address" {
  value = "${data.azurerm_public_ip.pip_ci2.ip_address}"
}
